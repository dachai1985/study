...
# 获取当前Excel所有sheet列表
def get_excel_sheet_list_pd(file_name):
    try:
...
def get_bts_count_sheet(file_name, sheet_name):
    ...
    if position is None:
        # column_name_to_check = 'BTS_x000D_\nNo'
        column_name_to_check = 'BTS\r\nNo'
        position = find_position_with_string(sheet, column_name_to_check)
    if position is None:
        column_name_to_check = 'BTS\nNo'
        position = find_position_with_string(sheet, column_name_to_check)
    ...
    if position is not None:  
        ...
        # 解包元组获取列索引和行号  
        column_index, row_number = position  
        bts_count_sheet = 0
        # 获取指定列的列字母  
        column_letter = openpyxl.utils.get_column_letter(column_index)
        # 遍历指定列的所有单元格  
        # for cell in sheet[openpyxl.utils.get_column_letter(position)]:
        # 从匹配项所在行以下的单元格开始遍历  
        for cell in sheet[column_letter][row_number:]:
            if cell.value and str(cell.value).strip() != '':
                bts_count_sheet += 1
        # bts_count_sheet = bts_count_sheet - 1
...
if __name__=='__main__':
    reading_folder = r'C:\\Users\\win\\Desktop\\test\\games'
