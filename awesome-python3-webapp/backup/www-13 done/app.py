...
from aiohttp.web_middlewares import middleware; logging.basicConfig(level=logging.INFO)
...
async def logger_factory(app, handler):
    async def logger(request):
        logging.info('Request: %s %s' % (request.method, request.path)) # method 字符拼错，浏览器访问时报501
...
async def init(loop):
    ...
    srv = await loop.create_server(apprunner.server, '127.0.0.1', 9000) #app.make_handler()已废弃，替换为apprunner.server
